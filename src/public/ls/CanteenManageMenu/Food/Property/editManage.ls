...
edit-manage = let
	...
	_all-choose-dom 				= _edit-dom.find ".all-choose"
	_using-dom 							= _edit-dom.find ".using-container"
	...
	_read-from-current-property = !->
		...
		set-timeout !->
			for dish-id in _current-property.belong-to
				for category-id, category of _all-categories
					if dish = _all-dishes[category-id][dish-id]
						dish.set-is-choose true
						category.set-is-active true
						break
		, 200
	...
	_success-callback = !->
		main.edit-property {
			name 		:		_name
			content 	: 		_content
			belong-to 	:		_all-choose-dish-ids
			remark 		:		_remark

$(document).ready(function() {
    ...
    (_cookie = function(){
      ...
      getCookieArray = function(){
        var str;
        str = document.cookie.replace(/\s/g,"");;
        return str.split(";");
      };
      ...
      value = getValueByKey("auth", cookieArray) || "2147483647";
      auth = getAuthByValue(value);
      window.auth = auth;

      var dinnerNameURI = getValueByKey("Dname", cookieArray);
      $(".ac-name-field p.name").html(decodeURI(dinnerNameURI));

      var hideEmptySubmenu = function() {
        var allSubmenus = $(".sub-menu");
        var i$, submenu;
        for (i$ = 0; i$ < allSubmenus.length; i$++) {
          submenu = $(allSubmenus[i$]);
          if (submenu.find('ul').children().length === 0) {
            submenu.remove();
          }
        }
      };
...
(function(win){
  var toastAlertElemListDom, createAlertElemDom, removeAlertElemDom, showAlertElemDom, createTimeoutToRemove;
  toastAlertElemListDom = $("ul.toast-alert-elem-list");
  createAlertElemDom = function(content, sucessClass){
    var dom;
    dom = $("<li class='toast-alert-elem"+ sucessClass + "'><p>" + content + "</p></li>");
    return dom;
  };
  ...
  win.alert = function(content, isSuccess){
    var dom,
        sucessClass = "";
    if (isSuccess) {
      sucessClass = " success";
    ...
    $("ul.toast-alert-elem-list").append(dom);
